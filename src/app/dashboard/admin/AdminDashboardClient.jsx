...
const CustomTooltip = ({ active, payload, label }) => {
  ...
  return (
    <div className="bg-background border border-border/40 rounded-xl px-4 py-3 shadow-lg space-y-1">
      <p className="font-body text-[11px] text-muted uppercase tracking-wider">{label}</p>
      {payload.map((p) => (
        <p key={p.dataKey} className="font-heading text-sm font-bold" style={{ color: p.color }}>
          {p.dataKey === "revenue" ? `$${Number(p.value).toLocaleString()}` : p.value}
          <span className="font-body text-xs font-normal text-muted ml-1">{p.name}</span>
        </p>
      ))}
    </div>
  );
...
const StatCard = ({ label, value, icon, color, bg, border, delay }) => (
  ...
  >
    ...
    <div className="min-w-0">
      ...
      <p className={`font-heading text-2xl sm:text-3xl font-bold mt-0.5 ${color}`}>{value}</p>
    </div>
  </motion.div>
);
    ...
    <div className={`rounded-xl border ${border} ${bg} p-4 flex items-center gap-3`}>
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center justify-between mb-1.5">
          ...
          <span className={`font-heading text-sm font-bold ${color}`}>{count}</span>
        </div>
        <div className="w-full h-1.5 bg-border/30 rounded-full overflow-hidden">
          <motion.div
            ...
            style={{ backgroundColor: color.includes("emerald") ? "#10b981" : color.includes("amber") ? "#f59e0b" : "#ef4444" }}
          />
        </div>
        <p className="font-body text-[10px] text-muted mt-1">{pct}% of all properties</p>
      </div>
    </div>
  );
};
...
export default function AdminDashboardClient({ analytics }) {
  ...
  return (
    <div className="p-4 max-w-7xl mx-auto space-y-8 pb-16">
      ...
      <motion.div {...fadeUp(0.3)} className="bg-surface rounded-3xl border border-border/40 shadow-sm p-5 sm:p-8">
        <div className="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
          <div>
            ...
            <p className="font-body text-xs text-muted mt-0.5">Revenue & bookings over the last 12 months</p>
          </div>
          <div className="flex items-center gap-4 text-[11px] font-body">
            <span className="flex items-center gap-1.5 text-muted">
              <span className="w-3 h-0.5 bg-[#043927] inline-block rounded-full" /> Revenue
            </span>
            <span className="flex items-center gap-1.5 text-muted">
              <span className="w-3 h-0.5 bg-purple-500 inline-block rounded-full" /> Bookings
            </span>
          </div>
        </div>
          ...
          <ResponsiveContainer width="100%" height={300}>
            <AreaChart data={monthlyStats} margin={{ top: 5, right: 10, left: 0, bottom: 5 }}>
              ...
              <Area
                ...
                dot={{ r: 3.5, fill: "#043927", strokeWidth: 2, stroke: "#fff" }}
                activeDot={{ r: 5.5, fill: "#043927", stroke: "#fff", strokeWidth: 2 }}
              />
              <Area
                yAxisId="bookings"
                type="monotone"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        {/* User Breakdown */}
        <motion.div {...fadeUp(0.4)} className="bg-surface rounded-3xl border border-border/40 shadow-sm p-5 sm:p-6 space-y-5">
          <div>
            ...
            <p className="font-body text-xs text-muted mt-0.5">Registered platform members</p>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-between py-3 border-b border-border/20">
              <div className="flex items-center gap-2.5">
                ...
                <span className="font-body text-sm text-foreground font-medium">Total Users</span>
              </div>
              <span className="font-heading text-xl font-bold text-blue-600">{totalUsers}</span>
            </div>
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2.5">
                ...
                <span className="font-body text-sm text-foreground font-medium">Owners</span>
              </div>
              <div className="text-right">
                <span className="font-heading text-xl font-bold text-secondary">{totalOwners}</span>
                <p className="font-body text-[10px] text-muted">
                  {totalUsers > 0 ? Math.round((totalOwners / totalUsers) * 100) : 0}% of users
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2.5">
                ...
                <span className="font-body text-sm text-foreground font-medium">Tenants</span>
              </div>
              <div className="text-right">
                <span className="font-heading text-xl font-bold text-purple-600">{totalTenants}</span>
                <p className="font-body text-[10px] text-muted">
                  {totalUsers > 0 ? Math.round((totalTenants / totalUsers) * 100) : 0}% of users
                </p>
              </div>
            </div>
        ...
        {/* Property Status */}
        <motion.div {...fadeUp(0.45)} className="bg-surface rounded-3xl border border-border/40 shadow-sm p-5 sm:p-6 space-y-5">
          <div>
            <h2 className="font-heading text-lg font-bold text-foreground tracking-tight">
              Property <span className="text-secondary">Status</span>
            </h2>
            <p className="font-body text-xs text-muted mt-0.5">Listing approval pipeline</p>
          </div>
        ...
        <motion.div {...fadeUp(0.5)} className="bg-surface rounded-3xl border border-border/40 shadow-sm p-5 sm:p-6 space-y-5">
          <div>
            <h2 className="font-heading text-lg font-bold text-foreground tracking-tight">
              Property <span className="text-secondary">Types</span>
            </h2>
            <p className="font-body text-xs text-muted mt-0.5">Listings by category</p>
          </div>
          ...
          {propertyTypeStats.length === 0 ? (
            ...
            <ResponsiveContainer width="100%" height={180}>
              ...
              >
                ...
                <XAxis
                  type="number"
                  tick={{ fontSize: 10, fontFamily: "inherit", fill: "var(--color-muted,#888)" }}
                  axisLine={false}
                  tickLine={false}
                  allowDecimals={false}
                />
                  ...
                  content={({ active, payload }) =>
                    active && payload?.length ? (
                      <div className="bg-background border border-border/40 rounded-lg px-3 py-2 shadow-md">
                        <p className="font-body text-xs text-muted">{payload[0]?.payload?.type}</p>
                        <p className="font-heading text-sm font-bold text-secondary">{payload[0]?.value} listings</p>
                      </div>
                    ) : null
                  }
                />
                <Bar dataKey="count" name="Listings" fill="#043927" radius={[0, 6, 6, 0]} />
              </BarChart>
            </ResponsiveContainer>
          )}
        </motion.div>
